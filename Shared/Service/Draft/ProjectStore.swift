...
private extension FileManager {
    ...
    func url(folder: ProjectStore.Folder) -> URL {
        guard var urlPath = self.urls(for: .documentDirectory, in: .userDomainMask).first else {
            fatalError("user don't have self folder")
        }
        
        urlPath.appendPathComponent(ProjectStore.Folder.storage.rawValue, isDirectory: true)
    ...
    func createIfNotExist(folder: ProjectStore.Folder) {
