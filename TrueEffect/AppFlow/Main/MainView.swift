...
struct MainView: View {
    ...
    @StateObject
    var viewModel = ViewModel(draftStore: ServiceLayer.shared.draftStore)
    ...
    private func grid(size: CGSize) -> some View {
        ...
        return LazyVGrid(columns: columns) {
            ForEach(0..<viewModel.count, id: \.self) { id in
                DraftImageView(draftStore: ServiceLayer.shared.draftStore, fileName: viewModel.drafts[id], size: cellSize)
